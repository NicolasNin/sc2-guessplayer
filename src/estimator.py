		...
		if liste:
			...
			for p in data.players:
				for g in data.players[p]:
					self.liste_of_games.append(g)
	def predict(self,game,method="manhattan",option=2,output="basic",race=True,maxgap=15,coefMat=1,coefGap=1,coefApm=1,coefFreq=1):
		...
		for g in self.liste_of_games:
			a=self.score(g,game,method,option,maxgap,coefMat,coefGap,coefApm,coefFreq)
			if mini>a:
				mini=a
				game_best=g
		if output=="basic":		
			...
			return (mini,game) 
	def predictByPlayer(self,game,method="manhattan",option=2,maxgap=15,coefMat=1,coefGap=1,coefApm=1,coefFreq=1,output="basic",race=True):			
		...
		if output=="basic":	
			...
			return (mini,game) 
	def giveProba(self,game,method="manhattan",option=2,maxgap=15,coefMat=1,coefGap=1,coefApm=1,coefFreq=1):
